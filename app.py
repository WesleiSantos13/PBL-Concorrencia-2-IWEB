...
def transferencia_pix(bank):
    ...
    # Supondo que obter_contas_vinculadas seja uma função que retorna as contas vinculadas ao CPF/CNPJ
    cpf_ou_cnpj = request.args.get('cpf_ou_cnpj')
    contas, erro = obter_contas_vinculadas(cpf_ou_cnpj, bank)
    if erro:
        flash('Erro ao obter contas: ' + erro, 'error')
        return redirect(url_for('dashboard', bank=bank))
...
if __name__ == '__main__':
    app.run(port=9998, host='172.31.96.1', debug=True)
